...
export default function BrandForm() {
  ...
  const [generatedIcons, setGeneratedIcons] = useState<{ url: string; id: string; }[]>([]);
  ...
  const onSubmit = async (data: BrandFormValues) => {
    ...
    try {
      const result = await generateIcons(data);
      
      if (!result.success || !result.images) {
        toast({
          title: "Generation Failed",
          description: result.error || "Failed to generate icons. Please try again.",
          variant: "destructive"
        });
        return;
      }

      setGeneratedIcons(result.images);
      
      // Show remaining credits
      if (result.creditsRemaining !== undefined) {
        toast({
          title: "Icons Generated Successfully",
          description: `You have ${result.creditsRemaining} credits remaining for today.`
        });
      }
